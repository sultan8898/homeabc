...
display_warning() {
    echo "WARNING: This script should not be run as the root user or master user."
}

# Check if the user is root or master
if [[ $(whoami) == "root" || $(whoami) == "master" ]]; then
    display_warning
    exit 1
fi

# Display warning not to run as platformops or master user
echo "WARNING: DO NOT RUN AS PLATFORMOPS OR MASTER USER"

# Prompt user if they are running the script as root
read -p "Are you running this script as root user? (yes/no): " answer

# Check if the user answered 'yes', then proceed with the script
if [[ "$answer" == "yes" ]]; then
...
EOF
    ...
    #CODE FOR REMOVING THE TXT FILE
    if [ -e 1_random_redis_file.txt ]; then
        rm -f 1_random_redis_file.txt
        echo "1_random_redis_file.txt deleted."
    else
        echo "1_random_redis_file.txt does not exist."
    fi
...
PHP
    ...
    echo "Script not executed. Please run as root user."
fi
